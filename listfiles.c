...
/**
 * print_list_array - returns an array of strings of the list->str
...
size_t print_linked_list(const list_t *h)
{
	...
	while (h)
	{
		input_string(convert_digit(h->num, 10, 0));
		_putchar(':');
		_putchar(' ');
		input_string(h->str ? h->str : "(nil)");
		input_string("\n");
		h = h->next;
		i++;
	}
...
/**
 * find_node_indexing - gets the index of a node
